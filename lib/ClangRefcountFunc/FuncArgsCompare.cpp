#include "ClangRefcountFunc/FuncArgsCompare.h"
...
bool ArgTypeCallback::setKeyVal(const clang::SourceManager &SM, const CallExpr *node,
    ...
    /**
     * 애초에 file이 겹치지 않게 사전작업을 해야 함
     * 각 type (field member, 순수 refcount) 별로 나누어서 총 4개의 vector를 생성
     * vector는 filename, line을 담고 있다. => 여기서 filename, line은 함수 call 기준임.
     * vector의 print() 메소드에 의해 log file에
     * 각 filename, line, 그리고 총 개수 size를 출력하게 된다.
     */
