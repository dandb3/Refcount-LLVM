...
#define TEST_DIR "/home/jdoh/Desktop/linux-v6.6/"
#define BUILD_LOG TEST_DIR "build.log"
#define CMDS_PATH TEST_DIR "compile.sh"
...
static void prepareCMD(Command &C, std::vector<std::string> &Objs, std::ofstream &Cmds) {
    for (auto it = C.args.begin(); it != C.args.end(); ++it) {
        if (it->size() == 3 && !strncmp(it->c_str(), "-O", 2)) {
            *it = "-O0";
        }
        else if (it->size() == 2 && !strncmp(it->c_str(), "-o", 2)) {
            ++it;
