...
#define LOG_DIR "/home/junwoong/work/refcount/build_compare_new/log/"
...
void initialize(std::vector<std::string> &totalFiles) {
    ...
    GS.dupStructLog = new raw_fd_ostream(llvm::StringRef(LOG_DIR "dup_struct_name.log"), errCode);
    GS.fileAccessLog = new raw_fd_ostream(llvm::StringRef(LOG_DIR "file_access.log"), errCode);

    if (GS.dupStructLog == nullptr || GS.fileAccessLog == nullptr) {
